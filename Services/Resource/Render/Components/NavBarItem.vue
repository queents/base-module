<script setup>
import { mdiChevronUp, mdiChevronDown, mdiAbjadArabic, mdiAlphaEBox } from '@mdi/js';
...
<template>
    ...
    >
        ...
        >
            <i :class="item.icon" v-if="itemLabel !== 'main'"></i>
            <span
                class="px-2 transition-colors"
                :class="{ 'lg:hidden': item.isDesktopNoLabel && item.icon }"
                >{{ itemLabel === 'main' ? user ? user.name : 'Profile' : trans(itemLabel) }}</span
            >
            <BaseIcon
                v-if="item.menu"
